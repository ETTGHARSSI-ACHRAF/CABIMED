...
const UpdateMutuelle = ({mutuelle}) => {
    const [open, setOpen] = React.useState(false);
    const [type, setType] = useState(mutuelle.type_mutuelle)

    const Update = (id) =>{
      
        axios.patch(`http://localhost:8000/api/mutuelle/${id}`,{
          type: type,
      })
      .then(res => {
        setOpen(false);
                        ...
                        <div className="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all max-w-lg w-full max-h-full p-8">
                            ...
                            <div className="p-6 space-y-6">
                                <div className="w-full">
                                    <input type="text" placeholder="Type" onChange={(e)=>{setType(e.target.value)}} defaultValue={mutuelle.type_mutuelle} className="px-4 py-2 bg-gray-50 w-full rounded-md" />
                                </div>
                                <div className="flex items-center p-6 space-x-2 rounded-b border-t border-gray-200 dark:border-gray-600">
                                    <button data-modal-toggle="default-modal" type="button" className="text-white bg-sky-500 hover:bg-sky-600 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center" onClick={()=>{Update(mutuelle._id)}} >Update</button>

...
const UpdateMedicament = ({medicament},getMedecaments) => {
  const [nom_med, setNom_med] = useState(medicament.nom_med)
  const [forme, setForme] = useState(medicament.forme_med)
  const [classe, setClasse] = useState(medicament.classe_med)
  const [open, setOpen] = useState(false)
  const navigate = useNavigate();
  const Update = (id) =>{
    axios.patch(`http://localhost:8000/api/medicament/${id}`,{
      nom: nom_med,
      forme: forme,
      classe: classe,
  })
            ...
            <div className="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all max-w-lg w-full max-h-full p-8">
              ...
              <div className="p-6 space-y-6">
                <div className="w-full">
                  <input type="text" placeholder="Nom" onChange={(e)=>{setNom_med(e.target.value)}} defaultValue={medicament.nom_med} className="px-4 py-2 bg-gray-50 w-full rounded-md" />
                </div>
                <div className="w-full">
                  <input type="text" placeholder="Forme" onChange={(e)=>{setForme(e.target.value)}} defaultValue={medicament.forme_med} className="px-4 py-2 bg-gray-50 w-full rounded-md" />
                </div>
                <div className="w-full">
                  <input type="text" placeholder="Classe" onChange={(e)=>{setClasse(e.target.value)}} defaultValue={medicament.classe_med} className="px-4 py-2 bg-gray-50 w-full rounded-md" />
                </div>
              </div>

              <div className="flex items-center p-6 space-x-2 rounded-b border-t border-gray-200 dark:border-gray-600">
                <button data-modal-toggle="default-modal" type="button" className="text-white bg-sky-500 hover:bg-sky-600 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center" onClick={()=>{Update(medicament._id)}} >Update</button>

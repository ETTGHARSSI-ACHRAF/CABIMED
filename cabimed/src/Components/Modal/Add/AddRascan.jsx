...
const AddRascan = () => {
    ...
    const addRascan = () => {
      axios.post('http://localhost:8000/api/rascan', {
        type: type,
        nom: nom
      })
        .then(res => {
  ...
  return (
    <>
      <button className="p-2 mr-2 border-sky-500 border text-sky-500 rounded transition duration-300 hover:bg-sky-500 hover:text-white focus:outline-none" onClick={() => setOpen(true)}>Ajouter Rascan</button>
      {open &&
            ...
            <div className="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all max-w-lg w-full max-h-full p-8">
              ...
              <div className="p-6 space-y-6">
                <div className="w-full">
                  <input type="text" placeholder="Nom" onChange={(e)=>{setNom(e.target.value)}} className="px-4 py-2 bg-gray-50 w-full rounded-md" />
                </div>
                <div className="w-full">
                  <input type="text" placeholder="Type" onChange={(e)=>{setType(e.target.value)}} className="px-4 py-2 bg-gray-50 w-full rounded-md" />
                </div>
              </div>

              <div className="flex items-center p-6 space-x-2 rounded-b border-t border-gray-200 dark:border-gray-600">
                <button data-modal-toggle="default-modal" type="button" className="text-white bg-sky-500 hover:bg-sky-600 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center" onClick={()=>{addRascan()}} >Ajouter</button>
